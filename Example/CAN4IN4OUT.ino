...
//Module pins available for use are any of the pins marked as "Not Used" in the Pin Use Map.
...
void processStartOfDay() {
  if (nextSodSwitchIndex >= 0
    ...
    if (++nextSodSwitchIndex >= NUM_SWITCHES) {
      DEBUG_PRINT(get_core_num() << F("> Done  all SOD events."));
      nextSodSwitchIndex = -1;
    } else {
      DEBUG_PRINT(get_core_num() << F("> Prepare for next SOD event."));
      nextSodMessageTime = millis() + SOD_INTERVAL;
    }
  }
}
